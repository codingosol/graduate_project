...
# 모든 채널의 공통 커버 기간을 이 날짜로 통일한다.
#
# 2026-08-05 변경: 2026-04-21(약 3개월) → 2025-08-01(약 1년). Neon 0.5GB에서 CockroachDB
# 10GB로 이전해 용량 여유가 생겨 커버 기간을 1년으로 확장했다. 2025-08은 이재명 취임
# (2025-06-04) 이후라 라벨링 규칙("대통령"=이재명)이 유지된다 — 이보다 더 내리면 윤석열
# 정권기가 섞여 기존 라벨을 전부 재검토해야 하므로 내리지 말 것(경위: DECISIONS.md, Part1.md §2).
# ⚠️ 업로드가 많은 채널은 playlistItems 최근 ~2만 편 한계로 2025-08까지 못 닿을 수 있고,
#    그 경우 backfill이 도달 지점을 최고령으로 인정해 exhausted 처리한다(YTN·연합과 동일).
RETENTION_FLOOR = "2025-08-01"
